...
- ⊘ CONN-1: Stand up a **remote** MCP transport (Streamable HTTP — current build is stdio) + OAuth, then submit to the **Claude Connector Directory** (`claude.com/docs/connectors/building/submission`, ~2wk review). Needs privacy-policy URL + production-ready status. Complementary to ToolHive (self-hosted/federal); this is the mainstream Claude-user funnel.
